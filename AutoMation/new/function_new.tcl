...
#::hostname# set IXIA_IP [lindex $argv 0]
#::port# set IXIA_Ports [lindex $argv 1]
#::mode# set Function_Name [lindex $argv 2]
#::detail# set Packet_Size [lindex $argv 3]
# set DurationMS [lindex $argv 4]
#::masterslave# set MasterSlave [lindex $argv 5]            
#::csport# set CableRelay_Port [lindex $argv 6]
#::portspeed# set PortSpeed [lindex $argv 7]        
...
set ::DurationMS [lindex $argv 4]
set ::MasterSlave [lindex $argv 5]
set ::CableRelay_Port [lindex $argv 6]
set ::PortSpeed [lindex $argv 7]
set ::Number [expr [lindex $argv 8]]
set ::summary ""
set ::summary_result ""
...
proc summary {{PacketSize 64} {RunTime 30}} {
	...
	set RunTime $RunTime
	set a "Masterslave,Length,Portspeed,Number,LinkUp_duration,PacketSize,RunTime"
	...
	if {[file exists $::Function_Name\_Summary.csv]==1} {
		...
		puts $fp1 "$::MasterSlave,$cable_length,$::PortSpeed,$::Number,$::duration,$PacketSize,$RunTime$::summary_result"
		close $fp1
...
if {$::Function_Name == "cableplug"} {
	...
	set_IXIA_stream_Continous_Packet $::Packet_Size
...
if {$::Function_Name == "forwarding"} {
	...
	if {$::Packet_Size == "random"} {
		set_IXIA_stream_Continous_Packet 64 1 64 1518 5
	} else {
		set_IXIA_stream_Continous_Packet $::Packet_Size
	}	
	getLinkState
...
# if {$::Function_Name == "forwarding" && $::detail == "hour"} {
	# set ::fp [open "$::Function_Name.txt" a+]
	# set ::log [logger::init Forwarding]
	# init
	# set_IXIA_Hardware
	...
	# set_IXIA_stream_Continous_Packet 1518 0 64 1518 4
	# getLinkState
	# learning_process
	# after 2000
	# getLinkState
	# Start_Traffic_And_Capture 3600000
	# #Report_LOOPBACK
	# Report_Nomal
	# summary 1518 "1 hour"
	# close $::fp 
...
# if {$::Function_Name == "forwarding" && $::detail == "reang"} {
	# set ::fp [open "$::Function_Name.txt" a+]
	# set ::log [logger::init Forwarding]
	# init
	# #set_IXIA_Hardware
	...
	# getLinkState
	# learning_process
	# Start_Traffic_And_Capture 10000
	# #Report_LOOPBACK
	# Report_Nomal
	# summary 64 "10sec"
	# close $::fp 
